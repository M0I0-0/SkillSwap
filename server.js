// Cargas el JSON directamente como un objeto de JavaScript
...
function crearTransporter() {
  // 1ER CAMBIO: Cambiamos 'process.env' por 'config'
  const {
    GMAIL_USER,
    GMAIL_APP_PASSWORD,
    SMTP_HOST,
...
async function sendResetEmail(recipientEmail, resetLink) {
  ...
  if (!transporter) {
    ...
    console.warn(
      "   Configura GMAIL_USER + GMAIL_APP_PASSWORD en tu config.json",
    ); // Ajusté el texto del aviso
    console.warn(`   👉 Enlace de recuperación: ${resetLink}\n`);
    return;
  }

  // 2DO CAMBIO: Cambiamos 'process.env' por 'config'
  const from = config.GMAIL_USER || config.SMTP_USER;
...
io.on("connection", (socket) => {
  socket.on("unirse", async ({ nombre, sala }) => {
    ...
    let salaRow = await getQuery("SELECT id FROM chat_salas WHERE nombre = ?", [
      sala,
    ]).catch(() => null);
    if (!salaRow) {
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!match) {
      ...
      await runQuery("UPDATE usuarios SET intentos_fallidos = ? WHERE id = ?", [
        intentos,
        row.id,
      ]);
    ...
    return res
      .status(200)
      .json({ message: "Login exitoso", user: formatUserProfile(row) });
  } catch (err) {
...
app.post("/api/recuperar", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "No fue posible procesar la solicitud." });
  }
});
...
app.post("/api/reset-password", async (req, res) => {
  ...
  if (!token)
    return res
      .status(400)
      .json({ success: false, message: "Token inválido o faltante." });
  if (!isValidPassword(password))
    return res.status(400).json({
      success: false,
      message: `La contraseña debe tener al menos ${PASSWORD_MIN_LENGTH} caracteres.`,
  ...
  try {
    ...
    if (!user)
      return res
        .status(400)
        .json({ success: false, message: "El enlace no es válido." });
    if (!user.reset_exp || Number(user.reset_exp) < Date.now()) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "El enlace ha expirado." });
    }
    const hash = await bcrypt.hash(password, 10);
    await runQuery(
  ...
  try {
    ...
    if (usuario) {
      ...
      const normalizeMatch = (m) => ({
        ...
        schedule: m.schedule?.includes("null")
          ? "Horario por confirmar"
          : m.schedule || "Horario por confirmar",
      });

      return res.json({
        summary: {
          newMatches: recommended.length,
    ...
    return res.json({
      ...
      recommendedMatches: habilidades.slice(0, 3).map((h) => ({
        ...
        schedule: h.schedule?.includes("null")
          ? "Horario por confirmar"
          : h.schedule || "Horario por confirmar",
      })),
    ...
    return res
      .status(500)
      .json({ message: "No fue posible cargar el dashboard." });
  }
});
...
app.post("/api/habilidades", async (req, res) => {
  ...
  try {
    if (!email || !titulo)
      return res
        .status(400)
        .json({ message: "Email y título son obligatorios." });
    const usuario = await getQuery(
      "SELECT id FROM usuarios WHERE lower(correo) = lower(?)",
      [email],
    );
    ...
    return res
      .status(201)
      .json({ message: "Habilidad creada correctamente.", id: result.lastID });
  } catch (err) {
    console.error(err);
...
app.delete("/api/habilidades/:id", async (req, res) => {
  ...
  try {
    ...
    if (!hab)
      return res
        .status(404)
        .json({ message: "Habilidad no encontrada o no es tuya." });
...
app.get("/api/habilidades/:id/alumnos", async (req, res) => {
  ...
  try {
    const instructor = await getQuery(
      "SELECT id FROM usuarios WHERE lower(correo) = lower(?)",
      [email],
    );
    ...
    if (!hab)
      return res
        .status(404)
        .json({ message: "Habilidad no encontrada o no te pertenece." });
...
app.put("/api/inscripciones/:id/progreso", async (req, res) => {
  ...
  try {
    ...
    if (!inscripcion)
      return res
        .status(404)
        .json({ message: "Inscripción no encontrada o no autorizada." });

    const nuevoEstado = prog >= 100 ? "completado" : "activa";
    await runQuery(
      "UPDATE inscripciones SET progreso = ?, estado = ? WHERE id = ?",
      [prog, nuevoEstado, inscId],
    );

    // Notificar al alumno cuando pasa el 60% (puede calificar) o llega a 100%
    const progresoAnterior = inscripcion.progreso_actual || 0;
    const hab = await getQuery("SELECT titulo FROM habilidades WHERE id = ?", [
      inscripcion.habilidad_id,
    ]);
    ...
    if (progresoAnterior < 60 && prog >= 60) {
      await runQuery(
        ...
        [
          ...
          `¡Alcanzaste el 60% en "${hab?.titulo}"! Ya puedes calificarlo. ⭐`,
        ],
      );
    }
...
app.post("/api/solicitudes", async (req, res) => {
  ...
  try {
    ...
    const para = para_usuario_id
      ? await getQuery("SELECT id FROM usuarios WHERE id = ?", [
          para_usuario_id,
        ])
      : await getQuery(
          "SELECT id FROM usuarios WHERE lower(correo) = lower(?)",
    ...
    if (inversa) {
      ...
      await runQuery(
        `INSERT INTO solicitudes (de_usuario_id, para_usuario_id, mensaje, estado) VALUES (?, ?, ?, 'aceptada')`,
        [de.id, para.id, mensaje],
      );
      await runQuery(
        `INSERT INTO notificaciones (usuario_id, tipo, mensaje) VALUES (?, 'solicitud', 'Coincidencia mutua: ambos se enviaron solicitud de intercambio 🎉')`,
        [de.id],
      );
      await runQuery(
        `INSERT INTO notificaciones (usuario_id, tipo, mensaje) VALUES (?, 'solicitud', 'Coincidencia mutua: ambos se enviaron solicitud de intercambio 🎉')`,
      ...
      return res.status(201).json({
        message: "¡Coincidencia! Solicitud aceptada automáticamente.",
        estado: "aceptada",
      });
    }

    const result = await runQuery(
      `INSERT INTO solicitudes (de_usuario_id, para_usuario_id, mensaje) VALUES (?, ?, ?)`,
      [de.id, para.id, mensaje],
    );
    ...
    return res
      .status(201)
      .json({ message: "Solicitud enviada.", estado: "pendiente" });
  } catch (err) {
    console.error(err);
...
app.put("/api/solicitudes/:id", async (req, res) => {
  ...
  try {
    ...
    if (!usuario) return res.status(403).json({ message: "No autorizado." });
    await runQuery("UPDATE solicitudes SET estado = ? WHERE id = ?", [
      estado,
      id,
    ]);
...
app.put("/api/notificaciones/leer", async (req, res) => {
  ...
  try {
    ...
    if (!usuario)
      return res.status(404).json({ message: "Usuario no encontrado." });
    await runQuery("UPDATE notificaciones SET leida = 1 WHERE usuario_id = ?", [
      usuario.id,
    ]);
...
app.get("/api/users/me", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "No fue posible cargar el perfil." });
  }
});
...
app.put("/api/users/me", async (req, res) => {
  ...
  const semesterVal =
    ...
    typeof req.body.semester === "number"
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "No fue posible actualizar el perfil." });
  }
});
...
app.put("/api/inscripciones/baja", async (req, res) => {
  ...
  if (!email || !habilidad_id)
    return res
      .status(400)
      .json({ message: "Email y habilidad_id requeridos." });
  try {
    const usuario = await getQuery(
      "SELECT id FROM usuarios WHERE lower(correo) = lower(?)",
      [email],
    ...
    if (!usuario)
      return res.status(404).json({ message: "Usuario no encontrado." });
    await runQuery(
      `UPDATE inscripciones SET estado = 'baja' WHERE usuario_id = ? AND habilidad_id = ?`,
      [usuario.id, habilidad_id],
    );
    const hab = await getQuery("SELECT titulo FROM habilidades WHERE id = ?", [
      habilidad_id,
    ...
    await runQuery(
      `INSERT INTO notificaciones (usuario_id, tipo, mensaje) VALUES (?, 'baja', ?)`,
...
app.post("/api/calificaciones", async (req, res) => {
  ...
  try {
    ...
    if ((inscripcion.progreso || 0) < 60)
      return res
        .status(403)
        .json({ message: "Debes completar al menos el 60% para calificar." });
    ...
    await runQuery(
      ...
      [
        usuario.id,
        `Calificaste "${hab?.titulo || "un curso"}" con ${estrellas} ⭐`,
      ],
    );
